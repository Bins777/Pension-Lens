"""
================================================================================
PensionLens — Non-Topological Baseline Classifiers
================================================================================
Trains and evaluates three strictly tabular baseline models:
  • Logistic Regression
  • Random Forest
  • XGBoost
...
---------------------------
This module intentionally uses ZERO graph-derived features.
The following columns produced by GDS / Neo4j graph algorithms are
explicitly blocked in TABULAR_FEATURES_BLOCKED so that the baselines
serve as a pure non-topological comparison point for the HeteroGNN:

  betweenness_score       ← GDS Betweenness Centrality
  community_concentration ← GDS Louvain Community Detection
  total_shared_managers   ← counts edges in the fund-fund graph
  in_circular_pattern     ← GDS Strongly Connected Components (SCC)

Input files (relative to BASE_PATH):
  pensionlens_labeled.csv          ← labeled fund feature matrix
  nodes_asset_managers_enriched.csv ← manager-level attributes

Outputs (written to OUTPUT_DIR):
  baseline_results_summary.csv     ← per-model metric table
  baseline_cv_scores.csv           ← cross-validation detail
...
Usage:
  python pensionlens_baseline.py
